...
def busqueda_pedidos_factor_multiple_con_cantidad_requerida():
    
    try:
        reglas= RelacionItemRegla.objects.filter(requiere_cantidad=True, factor="multiple")
        
        if reglas.exists():           
            for regla in reglas: 
                campo_busqueda = regla.objeto.tipo
                filtro = {campo_busqueda: regla.objeto.nombre, 'cantidad':regla.cantidad_condicion}
                pedidos = Acta.objects.filter(**filtro)
                if pedidos.exists():     
                    pedidos_a_evaluar_regla = pedidos.values('pedido').distinct()
                    evaluar_pedidos_regla_factor_multiple_con_cantidad_requerida(regla, pedidos_a_evaluar_regla, 0)
        
    except Exception as e:
        print(e)

def evaluar_pedidos_regla_factor_multiple_con_cantidad_requerida(regla, pedidos_a_evaluar, cantidad_requeridad):
