            ...
            if reg.item.tipo == "obra":
                ...
                for p in pedidos:
                    ...
                    if reg.comparador == "igual_a":
                        registro_actividad = Acta.objects.filter(pedido=p).first()
                        cantidad_item = Acta.objects.filter(pedido=p, item_cont=reg.item.nombre).aggregate(suma=Sum('cantidad'))['suma'] or 0

                        if reg.cantidad_requerida == 1 or reg.cantidad_requerida == cantidad_item:
                            if total != cantidad_regla:
                                nov = f"{reg.item.nombre} = {cantidad_regla}, {reg.item_requerido.nombre} = {total}"
                                if registro_actividad:
                                    crear_novedad(registro_actividad, nov)
